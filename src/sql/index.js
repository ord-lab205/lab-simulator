module.exports = {
  setup: {
    alterSession: [
      `ALTER SESSION SET nls_date_format = 'YYYY-MM-DD HH24:MI:SS'`, // 날짜 형식에 대한 세션 설정
    ],

    createSensors: tables => {
      let stmts = [];
      ...
      for (const table of tables) {
        stmts.push(...[
          ...
          `CREATE TABLE ${table} (idx NUMBER NOT NULL, data NUMBER, occurred DATE)`,
          `CREATE SEQUENCE ${table}_seq START with 1 INCREMENT BY 1 MAXVALUE 100 CYCLE NOCACHE`,
        ])
      }
    ...
    insertTestCases: (tables, values) => {
      const stmts = [];
      for (const table of tables) { // ['sound_sensor_a', 'sound_sensor_b', 'sound_sensor_c', 'vibration_sensor_a', 'vibration_sensor_b', 'vibration_sensor_c']
        for (const value of values) {
          stmts.push(`INSERT INTO ${table} values(${table}_seq.NEXTVAL, ${value}, SYSDATE)`);
        }
      }
    },
